...
public class LevelEndingManager : MonoBehaviour
{
    ...
    {
        Debug.Log("[LevelEnding] 레벨 모드 엔딩 완료!");
        
        // 데이터 로드 및 시청 완료 상태 저장
        ProgressData data = SaveManager.LoadData<ProgressData>("ProgressData");
        data.isLevelEndingViewed = true; 
        SaveManager.Save(data, "ProgressData");
        
        // 연출용 페이드 아웃 후 메인 홈으로 이동
        StartCoroutine(FadeOutAndComplete(() => {
            SceneManager.LoadScene("MainHome");
        }));
    }
    
    private IEnumerator FadeOutAndComplete(System.Action onFinished)
    {
        // 현재 다이얼로그가 있다면 페이드 아웃
        if (currentDialog != null)
        {
            yield return FadeOutDialog(currentDialog);
        }
        
        // 배경 페이드 아웃 (기존 EndingManager의 로직 활용)
        yield return FadeOutBackground();
